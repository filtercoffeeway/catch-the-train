...
def cfg() -> Config:
    return Config(
        telegram_token="t", allowed_chat=1, maps_key="m", bart_key="b", home_addr="home",
...
def planner():
    return Planner(cfg(), FakeBart(), FixedDrive(20 * MIN))


def test_to_office_leave_time_math():
    opts, errs = asyncio.run(planner().to_office(NOW, 4))
    assert errs == []
...
def test_to_office_skips_trains_you_cannot_make():
    opts, _ = asyncio.run(planner().to_office(hm(9, 45), 1))
    assert opts[0].trip.depart == hm(10, 28)


def test_to_home():
    opts = asyncio.run(planner().to_home(hm(14, 50), "UCTY", 4))
...
def test_alert_due_only_within_lead_and_once():
    opts, _ = asyncio.run(planner().to_office(hm(9, 20), 4))
